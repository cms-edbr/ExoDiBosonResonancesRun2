...
class EDBRTriggerStudies : public edm::EDAnalyzer {
public:
  explicit EDBRTriggerStudies(const edm::ParameterSet&);
  ~EDBRTriggerStudies();
...
private:
  ...
  int all,off,trg, mtch, gen, trg_g, mtch_g;
...
//
EDBRTriggerStudies::EDBRTriggerStudies(const edm::ParameterSet& iConfig)

{
  ...
  if(EDBRChannel_ == "VZ_CHANNEL")
    ...
    ex << "Unknown channel " << EDBRChannel_  
       << ". Please check EDBRTriggerStudies.cc for allowed values.";
    throw ex;
  }
  ...
  outTree_->Branch("mtch",&mtch,"mtch/I");
  outTree_->Branch("gen",&gen,"gen/I");  
...
EDBRTriggerStudies::~EDBRTriggerStudies()
{
...
bool EDBRTriggerStudies::electroncuts(vector<pat::Electron>::const_iterator theEl, reco::VertexCollection::const_iterator goodVertex, double rhoF){
  double pt = theEl->pt();
...
bool EDBRTriggerStudies::triggerfired(const edm::Event& ev, edm::Handle<edm::TriggerResults> triggerResultsHandle_, TString trigname){
...
bool EDBRTriggerStudies::triggerfound(const edm::Event& ev, edm::Handle<edm::TriggerResults> triggerResultsHandle_, TString trigname){
  const edm::TriggerNames TrigNames_ = ev.triggerNames(*triggerResultsHandle_);
  const unsigned int ntrigs = triggerResultsHandle_->size(); 
  for (unsigned int itr=0; itr<ntrigs; itr++){
    TString trigName=TrigNames_.triggerName(itr); 
...
unsigned int EDBRTriggerStudies::triggerIndex(const edm::Event& ev, edm::Handle<edm::TriggerResults> triggerResultsHandle_, TString trigname){
  const edm::TriggerNames TrigNames_ = ev.triggerNames(*triggerResultsHandle_);
  const unsigned int ntrigs = triggerResultsHandle_->size();
  unsigned int itr;
...
void
EDBRTriggerStudies::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  counter++;
  ...
  ls     = iEvent.eventAuxiliary().luminosityBlock();
   ...
   if (triggerFired) {
	 ...
	 //if (obj.id(92) && obj.pt()!=0){ 	 
	 // if ((obj.id(92) || (obj.id(81) && obj.id(92))) && obj.pt()!=0){ 	 
	 if (obj.pt()!=0){ 
...
void EDBRTriggerStudies::setDummyValues() {
     nVtx           = -1e9;
     triggerWeight  = -1e9;
     pileupWeight   = -1e9;
     lumiWeight     = -1e9;
...
void 
EDBRTriggerStudies::beginJob()
{
}

// ------------ method called once each job just after ending the event loop  ------------
void
EDBRTriggerStudies::endJob() {
  std::cout << "EDBRTriggerStudies endJob()..." << std::endl;
...
DEFINE_FWK_MODULE(EDBRTriggerStudies);
