...
void scanTrigReport(int channel){
   ...
   int vtx1 = t1->GetEntries( Form("index>=15 && lep==%d", channel) );
   int vtx2 = t2->GetEntries( Form("index>=15 && lep==%d", channel) );
   int vtx3 = t3->GetEntries( Form("index>=15 && lep==%d", channel) );
   int vtx4 = t4->GetEntries( Form("index>=15 && lep==%d", channel) );
   int vtx5 = t5->GetEntries( Form("index>=15 && lep==%d", channel) );
   int vtx6 = t6->GetEntries( Form("index>=15 && lep==%d", channel) );
   int vtx7 = t7->GetEntries( Form("index>=15 && lep==%d", channel) );
   int vtx8 = t8->GetEntries( Form("index>=15 && lep==%d", channel) );
   int vtx9 = t9->GetEntries( Form("index>=15 && lep==%d", channel) );
   int vtxA = tA->GetEntries( Form("index>=15 && lep==%d", channel) );
   int vtxB = tB->GetEntries( Form("index>=15 && lep==%d", channel) );
   int vtxC = tC->GetEntries( Form("index>=15 && lep==%d", channel) );

   int lep1 = t1->GetEntries( Form("index>=16 && lep==%d", channel) );
   int lep2 = t2->GetEntries( Form("index>=16 && lep==%d", channel) );
   int lep3 = t3->GetEntries( Form("index>=16 && lep==%d", channel) );
   int lep4 = t4->GetEntries( Form("index>=16 && lep==%d", channel) );
   int lep5 = t5->GetEntries( Form("index>=16 && lep==%d", channel) );
   int lep6 = t6->GetEntries( Form("index>=16 && lep==%d", channel) );
   int lep7 = t7->GetEntries( Form("index>=16 && lep==%d", channel) );
   int lep8 = t8->GetEntries( Form("index>=16 && lep==%d", channel) );
   int lep9 = t9->GetEntries( Form("index>=16 && lep==%d", channel) );
   int lepA = tA->GetEntries( Form("index>=16 && lep==%d", channel) );
   int lepB = tB->GetEntries( Form("index>=16 && lep==%d", channel) );
   int lepC = tC->GetEntries( Form("index>=16 && lep==%d", channel) );

   int dil1 = t1->GetEntries( Form("index>=19 && lep==%d", channel) );
   int dil2 = t2->GetEntries( Form("index>=19 && lep==%d", channel) );
   int dil3 = t3->GetEntries( Form("index>=19 && lep==%d", channel) );
   int dil4 = t4->GetEntries( Form("index>=19 && lep==%d", channel) );
   int dil5 = t5->GetEntries( Form("index>=19 && lep==%d", channel) );
   int dil6 = t6->GetEntries( Form("index>=19 && lep==%d", channel) );
   int dil7 = t7->GetEntries( Form("index>=19 && lep==%d", channel) );
   int dil8 = t8->GetEntries( Form("index>=19 && lep==%d", channel) );
   int dil9 = t9->GetEntries( Form("index>=19 && lep==%d", channel) );
   int dilA = tA->GetEntries( Form("index>=19 && lep==%d", channel) );
   int dilB = tB->GetEntries( Form("index>=19 && lep==%d", channel) );
   int dilC = tC->GetEntries( Form("index>=19 && lep==%d", channel) );

   int jet1 = t1->GetEntries( Form("index>=22 && lep==%d", channel) );
   int jet2 = t2->GetEntries( Form("index>=22 && lep==%d", channel) );
   int jet3 = t3->GetEntries( Form("index>=22 && lep==%d", channel) );
   int jet4 = t4->GetEntries( Form("index>=22 && lep==%d", channel) );
   int jet5 = t5->GetEntries( Form("index>=22 && lep==%d", channel) );
   int jet6 = t6->GetEntries( Form("index>=22 && lep==%d", channel) );
   int jet7 = t7->GetEntries( Form("index>=22 && lep==%d", channel) );
   int jet8 = t8->GetEntries( Form("index>=22 && lep==%d", channel) );
   int jet9 = t9->GetEntries( Form("index>=22 && lep==%d", channel) );
   int jetA = tA->GetEntries( Form("index>=22 && lep==%d", channel) );
   int jetB = tB->GetEntries( Form("index>=22 && lep==%d", channel) );
   int jetC = tC->GetEntries( Form("index>=22 && lep==%d", channel) );

   int jes1 = t1->GetEntries( Form("index>=25 && lep==%d", channel) );
   int jes2 = t2->GetEntries( Form("index>=25 && lep==%d", channel) );
   int jes3 = t3->GetEntries( Form("index>=25 && lep==%d", channel) );
   int jes4 = t4->GetEntries( Form("index>=25 && lep==%d", channel) );
   int jes5 = t5->GetEntries( Form("index>=25 && lep==%d", channel) );
   int jes6 = t6->GetEntries( Form("index>=25 && lep==%d", channel) );
   int jes7 = t7->GetEntries( Form("index>=25 && lep==%d", channel) );
   int jes8 = t8->GetEntries( Form("index>=25 && lep==%d", channel) );
   int jes9 = t9->GetEntries( Form("index>=25 && lep==%d", channel) );
   int jesA = tA->GetEntries( Form("index>=25 && lep==%d", channel) );
   int jesB = tB->GetEntries( Form("index>=25 && lep==%d", channel) );
   int jesC = tC->GetEntries( Form("index>=25 && lep==%d", channel) );

   int gra1 = t1->GetEntries( Form("index>=26 && lep==%d", channel) );
   int gra2 = t2->GetEntries( Form("index>=26 && lep==%d", channel) );
   int gra3 = t3->GetEntries( Form("index>=26 && lep==%d", channel) );
   int gra4 = t4->GetEntries( Form("index>=26 && lep==%d", channel) );
   int gra5 = t5->GetEntries( Form("index>=26 && lep==%d", channel) );
   int gra6 = t6->GetEntries( Form("index>=26 && lep==%d", channel) );
   int gra7 = t7->GetEntries( Form("index>=26 && lep==%d", channel) );
   int gra8 = t8->GetEntries( Form("index>=26 && lep==%d", channel) );
   int gra9 = t9->GetEntries( Form("index>=26 && lep==%d", channel) );
   int graA = tA->GetEntries( Form("index>=26 && lep==%d", channel) );
   int graB = tB->GetEntries( Form("index>=26 && lep==%d", channel) );
   int graC = tC->GetEntries( Form("index>=26 && lep==%d", channel) );
